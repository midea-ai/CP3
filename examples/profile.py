...
def profile_model(model, cfg):
    ...
    if cfg.get('flops', False):
        ...
        print(f'{cfg.batch_size}\t{N}\t{params / 1e6: .3f}\t{flops / (float(B) * 1e9): .5f}')
    ...
    if cfg.get('timing', False):
        ...
        writer = SummaryWriter('/data/private/project/pc/cls/PointNeXt/log/vis')
        writer.add_graph(model, *args)
        # with SummaryWriter(comment='Pointnet++') as w:
        #     w.add_graph(model, *args)
        with torch.no_grad():
