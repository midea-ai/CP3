...
import logging
from ..layers import furthest_point_sample, random_sample, LocalAggregation, three_interpolation, \
    create_convblock1d  # grid_subsampling,
from ..build import MODELS
from ..layers.local_aggregation import chip_LocalAggregation
import numpy as np
import random
import torch.nn.functional as F


class cd_PointNetSAModuleMSG(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(radii)):
            ...
            self.local_aggregations.append(
                chip_LocalAggregation(channels, aggr_args, conv_args, norm_args, act_args,
                                      group_args, use_res))
    ...
    def forward(self,
        ...
        if query_xyz is None and self.stride > 1:
            ...
            # dis points
            tmp_idx = idx
            sum_idx_test = np.array(random.sample(range(0, support_xyz.shape[1]), support_xyz.shape[1]))
            dis_idx = torch.zeros_like(idx)
            for i in range(idx.shape[0]):
                dis_idx[i] = torch.from_numpy(
                    sum_idx_test[~np.isin(sum_idx_test, tmp_idx[i].cpu().numpy())][:idx.shape[1]])
            dis_xyz = torch.gather(support_xyz, 1, dis_idx.unsqueeze(-1).expand(-1, -1, 3))
        else:
        ...
        for i in range(self.blocks):
            new_features, dis_new_features = self.local_aggregations[i](
                query_xyz, support_xyz, support_features, dis_xyz, query_idx=idx, dis_idx=dis_idx)
...
class cd_PointNetFPModule(nn.Module):
...
class chip_PointNet2Encoder(nn.Module):
...
@MODELS.register_module()
class chip_PointNet2Decoder(nn.Module):
...
@MODELS.register_module()
class chip_PointNet2PartDecoder(nn.Module):
    """PointNet++ MSG.
    """
