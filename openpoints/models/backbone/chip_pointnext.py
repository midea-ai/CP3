...
def reduced_1_norm(input, row_index, data_index):
    input[data_index, row_index, :] = torch.zeros(input.shape[-1])
    m = torch.norm(input[data_index, :, :], p='nuc').item()
    return m


def ci_score(feature):
    feature = feature.reshape(feature.shape[0], feature.shape[1], -1)

    r1_norm = torch.zeros([feature.shape[0], feature.shape[1]])
    for i in range(feature.shape[0]):
        for j in range(feature.shape[1]):
            r1_norm[i, j] = reduced_1_norm(feature.clone().detach(), j, data_index=i)
    ci = np.zeros_like(r1_norm)

    for i in range(r1_norm.shape[0]):
        original_norm = torch.norm(torch.tensor(feature[i, :, :].clone().detach()), p='nuc').item()
        ci[i] = original_norm - r1_norm[i]
    ci = np.mean(ci, axis=0)
    return torch.from_numpy(ci)
...
class chip_SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        self.chip_channel_norm = []

    def forward(self, pf):
        tmp_cof_channels_norm = self.cof_channels_norm
        tmp_dis_points_norm = self.dis_points_norm
        tmp_chip_channel_norm = self.chip_channel_norm
        self.cof_channels_norm = []
        self.dis_points_norm = []
        self.chip_channel_norm = []
        p, f = pf
        ...
        if self.is_head:
            f = self.convs(f)  # (n, c)
            chip_norm = ci_score(f)
            cof_sim = torch.mean(torch.min(F.normalize(torch.cdist(f, input_f)), -1)[0], 0)
            dis_norm = torch.sqrt(torch.sum(f.permute(0, 2, 1).contiguous() ** 2, dim=(0, 1)))
            self.cof_channels_norm.append(cof_sim)
            self.dis_points_norm.append(dis_norm)
            self.chip_channel_norm.append(chip_norm)
        else:
            if not self.all_aggr:
                idx = self.sample_fn(p, p.shape[1] // self.stride).long()
                sum_idx_test = np.array(random.sample(range(0, p.shape[1]), p.shape[1]))
                dis_idx = torch.zeros_like(idx)
                for i in range(idx.shape[0]):
                    dis_idx[i] = torch.from_numpy(
                        sum_idx_test[~np.isin(sum_idx_test, idx[i].cpu().numpy())][:p.shape[1] // self.stride])
                new_p = torch.gather(p, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
                dis_p = torch.gather(p, 1, dis_idx.unsqueeze(-1).expand(-1, -1, 3))
            else:
                new_p = p
                dis_p = p
            """ DEBUG neighbor numbers. 
            ...
            if self.use_res or 'df' in self.feature_type:
                f_mid = self.convs[0](fj)
                # mid_cof_sim = torch.mean(
                #     torch.mean(F.normalize(torch.cdist(self.pool(f_mid), new_p.permute(0, 2, 1).contiguous())), -1), 0)
                mid_chip_normal = ci_score(f_mid)
                # mid_cof_sim = torch.mean(
                #     torch.min(F.normalize(torch.cdist(self.pool(f_mid), new_p.permute(0, 2, 1).contiguous())), -1)[0],
                #     0)
                mid_cof_sim = torch.mean(
                    torch.max(F.normalize(torch.cdist(self.pool(f_mid), new_p.permute(0, 2, 1).contiguous())), -1)[0],
                    0)
                self.cof_channels_norm.append(mid_cof_sim)
                self.chip_channel_norm.append(mid_chip_normal)
                f_end = self.convs[1](f_mid)
                f = self.pool(f_end)
            else:
                f_mid = self.convs[0](fj)
                mid_chip_normal = ci_score(f_mid)
                mid_cof_sim = torch.mean(
                    torch.min(F.normalize(
                        torch.cdist(self.pool(f_mid.permute(0, 1, 3, 2)), new_p.permute(0, 2, 1).contiguous())), -1)[0], 0)
                self.cof_channels_norm.append(mid_cof_sim)
                self.chip_channel_norm.append(mid_chip_normal)
                f_end = self.convs[1](f_mid)
                end_chip_normal = ci_score(f_end)
                f = self.pool(f_end)
                # f_cof_sim = torch.mean(
                #     torch.min(F.normalize(
                #         torch.cdist(self.pool(f_end.permute(0, 1, 3, 2)), new_p.permute(0, 2, 1).contiguous())), -1)[0], 0)
                f_cof_sim = torch.mean(
                    torch.max(F.normalize(
                        torch.cdist(self.pool(f_end.permute(0, 1, 3, 2)), new_p.permute(0, 2, 1).contiguous())), -1)[0], 0)
                self.cof_channels_norm.append(f_cof_sim)
                self.chip_channel_norm.append(end_chip_normal)
            if self.use_res:
                f = self.act(f + identity)
                f_chip_nomal = ci_score(f)
                # f_cof_sim = torch.mean(
                #     torch.min(F.normalize(torch.cdist(f, new_p.permute(0, 2, 1).contiguous())), -1)[0], 0)
                f_cof_sim = torch.mean(
                    torch.max(F.normalize(torch.cdist(f, new_p.permute(0, 2, 1).contiguous())), -1)[0], 0)
                self.cof_channels_norm.append(f_cof_sim)
                self.chip_channel_norm.append(f_chip_nomal)
            p = new_p
            with torch.no_grad():
                if self.use_res or 'df' in self.feature_type:
                    dis_fi = torch.gather(
                        input_f, -1, dis_idx.unsqueeze(1).expand(-1, input_f.shape[1], -1))
                    if self.use_res:
                        dis_identity = self.skipconv(dis_fi)
                else:
                    dis_fi = None
                dis_dp, dis_fj = self.grouper(dis_p, p, input_f)
                dis_fj = get_aggregation_feautres(dis_p, dis_dp, dis_fi, dis_fj, feature_type=self.feature_type)
                if self.use_res or 'df' in self.feature_type:
                    dis_f_mid = (self.convs[0](dis_fj))
                    # dis_f_mid_norm = torch.sqrt(
                    #     torch.sum(self.pool(dis_f_mid).permute(0, 2, 1).contiguous() ** 2, dim=(0, 1)))
                    # dis_f_mid_norm = torch.mean(
                    #     torch.min(F.normalize(torch.cdist(self.pool(dis_f_mid), dis_p.permute(0,2,1).contiguous())), -1)[0],
                    #     0
                    # )
                    dis_f_mid_norm = torch.mean(
                        torch.max(F.normalize(torch.cdist(self.pool(dis_f_mid), dis_p.permute(0,2,1).contiguous())), -1)[0],
                        0
                    )
                    self.dis_points_norm.append(dis_f_mid_norm)
                    dis_f = self.convs[1](dis_f_mid)
                    dis_f = self.pool(dis_f)
                else:
                    dis_f_mid = (self.convs[0](dis_fj))
                    # dis_f_mid_norm = torch.sqrt(
                    #     torch.sum(self.pool(dis_f_mid).permute(0, 2, 1).contiguous() ** 2, dim=(0, 1)))
                    # dis_f_mid_norm = torch.mean(
                    #     torch.min(F.normalize(torch.cdist(self.pool(dis_f_mid.permute(0, 1, 3, 2)), dis_p.permute(0,2,1).contiguous())), -1)[0],
                    #     0
                    # )
                    dis_f_mid_norm = torch.mean(
                        torch.max(F.normalize(torch.cdist(self.pool(dis_f_mid.permute(0, 1, 3, 2)), dis_p.permute(0,2,1).contiguous())), -1)[0],
                        0
                    )
                    self.dis_points_norm.append(dis_f_mid_norm)
                    dis_f = self.convs[1](dis_f_mid)
                    tmp_dis_f = dis_f
                    dis_f = self.pool(dis_f)
                    # dis_f_end_norm = torch.sqrt(
                    #     torch.sum(self.pool(dis_f_mid).permute(0, 2, 1).contiguous() ** 2, dim=(0, 1)))
                    # dis_f_end_norm = torch.mean(
                    #     torch.min(F.normalize(torch.cdist(self.pool(tmp_dis_f.permute(0,1,3,2)), dis_p.permute(0,2,1).contiguous())), -1)[0],
                    #     0
                    # )
                    dis_f_end_norm = torch.mean(
                        torch.max(F.normalize(torch.cdist(self.pool(tmp_dis_f.permute(0,1,3,2)), dis_p.permute(0,2,1).contiguous())), -1)[0],
                        0
                    )
                    self.dis_points_norm.append(dis_f_end_norm)
                if self.use_res:
                    dis_f = self.act(dis_f + dis_identity)
                    # dis_f_end_norm = torch.sqrt(torch.sum(dis_f.permute(0, 2, 1).contiguous() ** 2, dim=(0, 1)))
                    # dis_f_end_norm = torch.mean(
                    #     torch.min(F.normalize(torch.cdist(dis_f, dis_p.permute(0,2,1))), -1)[0],
                    #     0
                    # )
                    dis_f_end_norm = torch.mean(
                        torch.max(F.normalize(torch.cdist(dis_f, dis_p.permute(0,2,1))), -1)[0],
                        0
                    )
                    self.dis_points_norm.append(dis_f_end_norm)
        if len(tmp_cof_channels_norm) == len(tmp_dis_points_norm) == len(tmp_chip_channel_norm) and len(tmp_chip_channel_norm) != 0:
            # print('add previous')
            for i in range(len(self.cof_channels_norm)):
                self.cof_channels_norm[i] = F.normalize((self.cof_channels_norm[i] + tmp_cof_channels_norm[i]), dim=0)
                self.dis_points_norm[i] = F.normalize((self.dis_points_norm[i] + tmp_dis_points_norm[i]), dim=0)
                self.chip_channel_norm = F.normalize((self.chip_channel_norm[i] + tmp_chip_channel_norm[i]), dim=0)

        else:
            for i in range(len(self.cof_channels_norm)):
                self.cof_channels_norm[i] = F.normalize(self.cof_channels_norm[i], dim=0)
                self.dis_points_norm[i] = F.normalize(self.dis_points_norm[i], dim=0)
                self.chip_channel_norm[i] = F.normalize(self.chip_channel_norm[i], dim=0)
...
class chip_PointNextEncoder(nn.Module):
    ...
    def _make_enc(self, block, channels, blocks, stride, group_args, is_head=False):
        ...
        layers.append(chip_SetAbstraction(self.in_channels, channels,
...
@MODELS.register_module()
class chip_PointNextDecoder(nn.Module):
    def __init__(self,
...
@MODELS.register_module()
class chip_PointNextPartDecoder(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
