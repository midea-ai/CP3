"""Modified by the official implementation of PointNext, which includes channel pruning at random rates.
"""
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
                 in_channels, mid_channel, out_channels,
                 layers=1,
                 stride=1,
...
class flops_PointNextEncoder(nn.Module):
    r"""
    Args:
    ...
    def __init__(self,
        ...
        for i in range(len(blocks)):
            ...
            encoder.append(self._make_enc(
                ...
                mid_channel=mid_channels[i - 1] if mid_channels is not None and i != 0 else None
            ))
    ...
    def _make_enc(self, block, channels, blocks, stride, group_args, is_head=False, mid_channel=None):
        layers = []
        radii = group_args.radius
        nsample = group_args.nsample
        group_args.radius = radii[0]
        group_args.nsample = nsample[0]
        layers.append(SetAbstraction(self.in_channels, mid_channel, channels,
